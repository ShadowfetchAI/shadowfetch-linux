...
RELEASE_DEBS := $(BUILD_DIR)/*_$(VERSION)-1_all.deb $(BUILD_DIR)/grub-btrfs_*_all.deb
...
.PHONY: all help deps packages repo iso sign pre-release-check publish qemu clean distclean \
...
repo: packages
	...
	@# Corresponding source. /linux/licensing carries a written offer that the
	@# complete corresponding source is published in main/source alongside the
	@# binaries, no request or fee. A release that ships binaries with no source
	@# makes that offer false, so the source packages are built here rather than
	@# by hand. Built from cleaned copies so build artifacts stay out of the
	@# tarballs; -T dsc keeps the removal from touching the binaries.
...
iso: repo
	...
	@bash -c 'cd "$(REPO_DIR)" && python3 -m http.server $(LOCAL_REPO_PORT) --bind 127.0.0.1 >"$(BUILD_DIR)/repo-server.log" 2>&1 & \
		SERVER_PID=$$! ; \
